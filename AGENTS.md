...
- Replay PETSc only in the matched external environment documented by `docs/PETSC_VI_ASSESSMENT.md`: `PETSC_DIR="$(brew --prefix petsc)" .venv-petsc/bin/python scripts/run_petsc_vi_assessment.py --verify`. A skipped petsc4py import is not support evidence.
