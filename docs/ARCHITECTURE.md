...
| `optional_imports` | Clean-wheel imports for `fd`, `jax`, `calibration`, `viz`, `ui`, `volatility`, `changepoints`, `quantlib`, and `identifiability` extras; the `quantlib` profile runs focused #132 oracle/state tests and the `identifiability` profile runs focused #133 profile-likelihood tests plus canonical artifact verification on Python 3.11 and 3.12 | Blocking for advertised extras |
