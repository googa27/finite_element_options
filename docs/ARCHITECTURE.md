...
Issue #130 adds an adoption-only boundary package at `finite_element_options.examples.regime_switching_quanto.adoption`. The facade is lazy; `optional.py` records JSON-safe registry entries for `arch`, `ruptures`, `QuantLib`, `iminuit`, and `openturns` and raises actionable `finite-element-options[extra]` install errors, while `quantlib_oracle/state.py` is a narrow internal adapter that serializes QuantLib `Settings.instance().evaluationDate` changes and restores the prior process-global date in `finally`. Issue #131 adds a bounded `volatility_benchmark` facade route and CLI that use arch and ruptures only when invoked, refuse immutable CSV hash mismatches, compare four arch challengers with a real statsmodels Markov AR(2) baseline on identical rolling hold-out blocks, and emit canonical JSON summaries without raw PDP time series or local input paths. The #131 report and artifact hash (`3ef33542865cc7370bc639b15b60aba207a2be3981ad77b9d1132f5f0e15f9ad`) are recorded in `docs/REGIME_VOLATILITY_CHALLENGERS.md`. Issue #132 adds a cohesive `quantlib_oracle` package beneath the adoption boundary for one-regime vanilla/fixed-FX quanto oracle parity against QuantLib, the repository Black-Scholes analytical oracle, existing one-regime FEM, and seeded MC. The #132 report and artifact hash (`ca2789e8f686a2f25b9abebc076f18ce7596673b038e52b681478cad22c4a056`) are recorded in `docs/REGIME_QUANTLIB_ORACLE.md`; this evidence does not validate the full multi-regime PDE. Issue #133 adds `identifiability/`, for iminuit-backed public-synthetic instrument-target calibration diagnostics. Contracts remain stdlib/date/JSON-safe, the maintained `Minuit` API owns `migrad`, `hesse`, `minos`, and bounded `mnprofile` scans, and raw iminuit objects never cross the adapter. The identification gate fails closed unless the minimum, EDM, bounds, HESSE covariance, standard errors, MINOS intervals, finite-difference curvature, and two-sided profile crossings all pass. The #133 report is `docs/REGIME_IMINUIT_IDENTIFIABILITY.md` with artifact hash `6294b52e9d6aa26aeda39a1809486272223d41ecc7a00e42e670f5dcbba39a3b`; public-synthetic instrument targets are not observed/live market prices or production calibration. Issue #134 moves the internal QuantLib state adapter into `quantlib_oracle/state.py` to preserve the adoption fan-out limit, then adds exactly one `uncertainty/` package for OpenTURNS-backed propagation/raw Saltelli sensitivity with confidence intervals. The #134 report is `docs/REGIME_OPENTURNS_UQ.md` with artifact hash `d488ea1d2300b3cd1da882479a5a475b22732145335ca3e4a3abd4393e80463f` and input hash `c334b7d7863b46df4aac3a8550aa52587090e79be7605f3f334fe1c224d9bf5b`; it is a public-synthetic estimator/validation diagnostic, not an intrinsic fair-value risk distribution. These extras (`volatility`, `changepoints`, `quantlib`, `identifiability`, `uncertainty`) are independent import profiles, not base dependencies and not numerical capability promotions.
...
Issue #150 adds the isolated Python 3.12 `jax-regime` research profile under `examples.regime_switching_quanto.jax_regime`. DYNAMAX owns the bivariate full-covariance Gaussian-HMM EM/filter/smoother, while a small independently parity-tested JAX forward scan analytically marginalizes latent states for NumPyro NUTS. Historical physical-measure parameters do not determine pricing drifts: the pricing route applies the existing constrained domestic-`Qd` convention and explicitly reuses the fitted physical transition matrix without claiming a calibrated regime-risk premium. Diffrax Euler is forced to every regime boundary and must match the conditionally exact JAX log-diffusion update pathwise; it is an interoperability/extension seam, not an accuracy upgrade. The facade and contracts remain dependency-light, PDP is consumed only through a caller-supplied content-addressed public export, and the hash-pinned profile freezes DYNAMAX's otherwise floating `tfp-nightly` dependency. Existing statsmodels/scikit-fem and NumPy/SciPy routes remain canonical rollback oracles.
...
American exercise uses the discrete lower-obstacle LCP convention
\[
...
Issue #74 publishes the public arxiv-lab contract artifacts in `tests/fixtures/fem_bs_001/` alongside the executable report: `problem_spec.json` (fixture/problem spec), deterministic mesh/time-step configuration and boundary metadata, `result_export.json` (convergence rows and summary), and explicit equal-error comparison policy for parity consumers. Serialization-boundary helpers live in `finite_element_options.validation.evidence.public_fixture`: they attach result hashes only after all adapter mutations, emit fixture provenance/PDE convention metadata, and canonicalize published numerical rows to a documented 13-significant-digit representation before JSON hashing. This canonicalization targets ULP-level SciPy/SuperLU serialization drift only and does not change the advertised price/Delta/Gamma acceptance tolerances.
...
Issue #116 adds `finite_element_options.validation.compiled_weak_form_adapter` and the public fixture `tests/fixtures/compiled_weak_form/black_scholes_call_v0.json`. The adapter consumes serialized FPF `pde_ir.v0` plus `compiled_symbolic_operator.v0` evidence as data, never through source-tree imports, and releases only the exact public-synthetic Black-Scholes line-uniform/Lagrange-P2/theta/SciPy-direct route. It fails closed before mesh allocation if privacy class, schema, source/compiled hashes, strong/weak-form sign, measure, numeraire, units, one-dimensional domain/time metadata, boundary essential/natural partition, outputs, mesh, element, solver controls, route parameters, terminal/boundary expressions, expression hashes or semantic evidence drift. The screen compares `pde_ir`, `compiled_operator` and FEM route subobjects to the packaged golden fixture, rejects nested unknown fields, and reports unsupported typed conversions as `CompiledWeakFormDiagnostic` values rather than raw exceptions. The `fem-options qps screen|solve` CLI emits deterministic JSON and writes machine-readable result/evidence artifacts.

Issue #117 adds `finite_element_options.validation.fem_evidence` as the versioned evidence-bundle generator for `fem-bs-001`. It records SymPy manufactured-source provenance, h/time convergence rows, perturbation-failure rows, Black-Scholes price/Delta/Gamma oracle rows, no-arbitrage evidence, backend/request/convention/mesh-time/result hashes, and a full evidence hash. The CLI route `fem-options validation run-benchmark fem-bs-001 --out PATH` validates before writing; without `--out` it emits the same validated JSON to stdout.
...
| Calibration and statistical workflows | Optional profile or downstream consumer; issue #45 provides bounded SciPy pricing calibration primitives, but production Heston evidence still requires an injected validated PDE/Fourier/oracle artifact. |
...
The minimal wheel includes only core numerical dependencies. `scikit-fem[all]` is not acceptable for core because it makes optional features mandatory. FEniCSx/PETSc platform constraints are documented and tested in dedicated environments. The issue #130 adoption libraries are split by purpose as `volatility` (`arch>=8,<9`), `changepoints` (`ruptures>=1.1.10,<2`), `quantlib` (`QuantLib>=1.43,<2`), and `identifiability` (`iminuit>=2.32,<3`). Import success for these profiles proves only the adapter boundary; the QuantLib profile also executes the #132 state/oracle tests, and the identifiability profile executes the #133 profile-likelihood tests, on Python 3.11 and 3.12 from clean installed-wheel environments. These optional jobs do not promote volatility modeling, changepoint detection, QuantLib oracle parity, or identifiability calibration to production capabilities.
...
Issue #42 adds `finite_element_options.validation.verification_gates` as the executable validation meta-layer behind `FEM-VALIDATION-GATES-V0`; declarative benchmark contracts and the fail-closed validation error now live in `finite_element_options.validation.evidence.benchmark_registry`, while compatibility re-exports preserve the established validation API. A `BenchmarkSpec` must name model, instrument, state convention, domain, grid, time schedule, oracle, norm, expected order and separated discretization/oracle/floating-point tolerance components before a capability record can cite it as evidence. `ConvergenceStudy` evaluates spatial, temporal and domain-truncation studies independently and raises with row-level actionable tables when observed order, final error budgets or non-finite tolerance components fail. `manufactured_solution_cases()` provides explicit smooth manufactured residual canaries for diffusion, convection-diffusion, mixed-derivative and state-dependent-reaction operators; unknown operators fail closed. `evaluate_call_arbitrage()` requires one contract surface (same strike/rate/maturity) and fails closed on duplicate spot coordinates, positivity/bounds, monotonicity, convexity, Delta and Gamma violations. `compare_backend_reports()` refuses cross-backend parity unless PDE-convention, grid and time-schedule hashes plus output metric sets match, and `evaluate_american_lcp_gate()` requires complementarity residuals plus a real exercise/continuation front transition. These gates deliberately stay lightweight; large QuantLib/FEniCSx work-precision studies can plug into the same report shapes later.

Issue #45 adds `CalibrationObjective`, `PricingCalibrationDataset`, `PricingModelCalibrator` and `HestonPricingCalibrator` as the diagnosable calibration boundary. Market data must carry spot, strike, maturity, rate, carry, quote units and optional bid/ask, explicit weights or vega before an objective can request bid-ask or vega scaling. Calibration uses bounded SciPy least squares, robust loss metadata, deterministic multi-start candidates, finite price/residual checks, holdout RMSE and optimizer/Jacobian diagnostics; a result is successful only when the optimizer and every pricing call succeed and fitted parameters remain inside declared bounds. Heston calibration does not ship a hidden toy pricer: callers inject a separately validated Heston pricing function plus a content-addressed validation artifact, and toy/synthetic/polynomial engine names fail closed while positivity/correlation/Feller policy diagnostics are attached to the result.
...
| `optional_imports` | Clean-wheel imports for `fd`, `jax`, `calibration`, `viz`, `ui`, `volatility`, `changepoints`, `quantlib`, and `identifiability` extras; the `quantlib` profile runs focused #132 oracle/state tests and the `identifiability` profile runs focused #133 profile-likelihood tests on Python 3.11 and 3.12 | Blocking for advertised extras |
...
Source of truth: `docs/ARCHITECTURE.yaml`. Tracking: [Project #24](https://github.com/users/googa27/projects/24), [finite_element_options issue](https://github.com/googa27/finite_element_options/issues/114). Profile: `strict`; enforcement: `Blocking`.
...
|---|---|---|---|
| Existing runtime stack | `numpy`, `pydantic`, `scikit-fem`, `scipy` | Reimplementation from scratch | Preserve public adapters; research maintenance/API/license before additions. |
...
- Human/notebook: Typed solver/weak-form API and notebooks; NumPy protocols for genuine arrays, lawful immutable value-object dunders only.
- Planned Python protocols: Immutable mesh/problem/result specs: deterministic __repr__ and explicit value semantics.; Mesh/function collections may use __iter__/__len__; pure forms/operators may use __call__.; Assembly, solve, adaptivity, calibration, I/O, and diagnostics remain named methods; NumPy protocols require shape/dtype tests.
- Core posture: Consume FPF contracts; avoid PDP/UI runtime dependencies; emit solver evidence bundles.
- Data posture: No data ownership; public-synthetic/convergence fixtures only.
