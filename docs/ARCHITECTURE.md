...
Issue #130 adds an adoption-only boundary package at `finite_element_options.examples.regime_switching_quanto.adoption`. The facade is lazy; `optional.py` records JSON-safe registry entries for `arch`, `ruptures`, `QuantLib`, and `iminuit` and raises actionable `finite-element-options[extra]` install errors, while `quantlib_state.py` is a narrow internal adapter that serializes QuantLib `Settings.instance().evaluationDate` changes and restores the prior process-global date in `finally`. Issue #131 adds a bounded `volatility_benchmark` facade route and CLI that use arch and ruptures only when invoked, refuse immutable CSV hash mismatches, compare four arch challengers with a real statsmodels Markov AR(2) baseline on identical rolling hold-out blocks, and emit canonical JSON summaries without raw PDP time series or local input paths. ARCH forecasts are deterministic one-step `method="analytic"` forecasts aligned to observed hold-out targets with `last_obs` fixed at the training boundary, so Student-t/skewed-t predictive scores use the fitted arch distribution rather than Gaussian simulation. Markov AR(2) baseline forecasts are also evaluated sequentially one-step-ahead: the statsmodels transition tensor is converted from next-by-previous orientation to an explicit row-stochastic matrix, prior regime probabilities are propagated before each forecast, AR(2) means use only observed training or earlier hold-out lags, and the current hold-out value updates the filter only after that score and exact Gaussian-mixture VaR are emitted. Baseline failure or nonconvergence fails closed and disables promotion even when diagnostic baseline metrics are present; full-sample Markov high-volatility regime-probability failures are serialized as typed changepoint comparison failures instead of zero-filled probabilities. The #131 report and artifact hash (`3ef33542865cc7370bc639b15b60aba207a2be3981ad77b9d1132f5f0e15f9ad`) are recorded in `docs/REGIME_VOLATILITY_CHALLENGERS.md`. Issue #132 adds a cohesive `quantlib_oracle` package beneath the adoption boundary: immutable stdlib/date/JSON-safe specs cover only TARGET/Actual365Fixed/Unadjusted/continuous-rate European calls, unsupported conventions fail with `QuantLibConventionError(field, received, supported)`, inconsistent vanilla reduction fields fail with `QuantLibReductionError(field, received, expected, kind)`, vanilla pricing uses `EuropeanOption` with `AnalyticEuropeanEngine`, fixed-FX quanto pricing uses `QuantoVanillaOption` with `QuantoEuropeanEngine`, and the deterministic one-regime matrix compares QuantLib, the repository Black-Scholes analytical oracle, existing one-regime FEM and seeded MC. The #132 report and artifact hash (`ca2789e8f686a2f25b9abebc076f18ce7596673b038e52b681478cad22c4a056`) are recorded in `docs/REGIME_QUANTLIB_ORACLE.md`; this evidence does not validate the full multi-regime PDE. These extras (`volatility`, `changepoints`, `quantlib`, `identifiability`) are independent import profiles, not base dependencies and not numerical capability promotions.
...
The minimal wheel includes only core numerical dependencies. `scikit-fem[all]` is not acceptable for core because it makes optional features mandatory. FEniCSx/PETSc platform constraints are documented and tested in dedicated environments. The issue #130 adoption libraries are split by purpose as `volatility` (`arch>=8,<9`), `changepoints` (`ruptures>=1.1.10,<2`), `quantlib` (`QuantLib>=1.43,<2`), and `identifiability` (`iminuit>=2.32,<3`). Import success for these profiles proves only the adapter boundary; the QuantLib profile now also executes the #132 state/oracle tests for one-regime vanilla and fixed-FX quanto reductions. These optional jobs do not promote volatility modeling, changepoint detection, QuantLib oracle parity, or identifiability calibration to production capabilities.
...
| `optional_imports` | Clean-wheel imports for `fd`, `jax`, `calibration`, `viz`, `ui`, `volatility`, `changepoints`, `quantlib`, and `identifiability` extras; the `quantlib` profile also runs the focused #132 oracle/state tests on Python 3.11 and 3.12 | Blocking for advertised extras |
