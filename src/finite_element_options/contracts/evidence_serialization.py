"""Dependency-light canonical serialization and provenance helpers."""
...
def json_safe(value: Any) -> Any:
    """Convert dataclasses, arrays, numeric scalars, and timestamps to JSON values."""
...
def distribution_install_mode(name: str) -> str:
    """Classify provenance while accepting only actual wheel installations as wheel."""
    ...
    if isinstance(directory, dict):
        return "editable" if directory.get("editable") is True else "directory"
    if isinstance(direct_url.get("vcs_info"), dict):
        return "vcs"
    if isinstance(direct_url.get("archive_info"), dict):
        source = unquote(urlparse(str(direct_url.get("url", ""))).path).lower()
        return "wheel" if source.endswith(".whl") else "archive"
    return "direct-url"
