...
Adoption-only optional libraries for this example are isolated behind the lazy `finite_element_options.examples.regime_switching_quanto.adoption` facade. Install only the purpose-specific profile needed for a research experiment: `volatility` for `arch>=8,<9`, `changepoints` for `ruptures>=1.1.10,<2`, `quantlib` for `QuantLib>=1.43,<2`, and `identifiability` for `iminuit>=2.32,<3`. Missing dependencies raise errors such as `install finite-element-options[quantlib]`; importing the facade or registry does not import the libraries eagerly. The issue #131 volatility benchmark runner combines `calibration`, `volatility`, and `changepoints` to compare arch GJR-GARCH/EGARCH challengers against the statsmodels Markov AR(2) baseline on a caller-supplied immutable CSV hash, writing only bounded canonical JSON summaries under `docs/evidence/`. The current #131 report is [docs/REGIME_VOLATILITY_CHALLENGERS.md](docs/REGIME_VOLATILITY_CHALLENGERS.md): artifact SHA `3ef33542865cc7370bc639b15b60aba207a2be3981ad77b9d1132f5f0e15f9ad`, decision `reject`, with promotion failed closed because the Markov AR(2) baseline did not converge. The issue #132 QuantLib oracle slice is documented in [docs/REGIME_QUANTLIB_ORACLE.md](docs/REGIME_QUANTLIB_ORACLE.md): it validates only one-regime vanilla and fixed-FX quanto reductions against QuantLib plus the repository analytical oracle, with artifact SHA `ca2789e8f686a2f25b9abebc076f18ce7596673b038e52b681478cad22c4a056`; it is not full multi-regime PDE validation. The issue #133 iminuit identifiability slice is documented in [docs/REGIME_IMINUIT_IDENTIFIABILITY.md](docs/REGIME_IMINUIT_IDENTIFIABILITY.md): it calibrates only literal public-synthetic fixed-FX quanto instrument targets, identifies the nonzero-FX-vol case, and rejects the fx_vol=0 weak-rho case, with artifact SHA `6294b52e9d6aa26aeda39a1809486272223d41ecc7a00e42e670f5dcbba39a3b`; it is not observed/live market or production calibration. QuantLib process-global evaluation dates are handled only inside the internal `quantlib_state` adapter and do not cross public result/domain contracts.
