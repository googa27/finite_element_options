...
Generated by `uv run --with matplotlib==3.10.3 --with pillow==12.3.0 python scripts/generate_readme_validation_visuals.py`; the deterministic JSON visual QA and provenance report is written to `docs/images/readme_visual_provenance.json`.
...
The separate [`jax-regime` research profile`](docs/JAX_REGIME_STUDY.md) fits a joint bivariate Gaussian HMM with DYNAMAX, estimates continuous-parameter uncertainty with NumPyro while analytically marginalizing latent states, and executes a regime-aligned Diffrax log-SDE route against exact-step and analytical oracles. It consumes the same hash-verified public PDP S&P 500/USDCLP export as the earlier study. Historical physical-measure transitions are reused under a constrained domestic pricing measure only as a named research assumption; the resulting prices are not option-implied, market-calibrated, or production-ready.

## Installation

The core wheel keeps optional stacks out of the base install. Use a local wheel-style install for consumers and an editable install constrained by `constraints.txt` for development gates. Optional profiles are published as extras: `fd`, `jax`, `jax-regime`, `calibration`, `volatility`, `changepoints`, `quantlib`, `identifiability`, `uncertainty`, `reduction`, `bayesian`, `bayesian-jax`, `io`, `viz`, `ui`, `validation`, `build`, and `dev`.
...
uv run --with matplotlib==3.10.3 --with pillow==12.3.0 python scripts/generate_readme_validation_visuals.py
