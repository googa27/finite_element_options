...
The separate [`reduction`](docs/BLACK_SCHOLES_PYMOR_ROM.md) profile provides an experimental pyMOR `>=2026.1,<2027` POD/Galerkin adapter for a narrowly validated variance-affine 1D Black–Scholes benchmark. Its canonical run achieved a `28.749x` median online speedup, met disjoint holdout price/Delta/Gamma tolerances, and amortized the offline cost to `10x` after 360 queries. It refuses volatility outside `[0.10, 0.35]` and names full-order FEM as fallback. This optional promotion does **not** add a capability-matrix row or establish a general ROM solver.
