...
Adoption-only optional libraries for this example are isolated behind the lazy `finite_element_options.examples.regime_switching_quanto.adoption` facade. Install only the purpose-specific profile needed for a research experiment: `volatility` for `arch>=8,<9`, `changepoints` for `ruptures>=1.1.10,<2`, `quantlib` for `QuantLib>=1.43,<2`, and `identifiability` for `iminuit>=2.32,<3`. Missing dependencies raise errors such as `install finite-element-options[quantlib]`; importing the facade or registry does not import the libraries eagerly. The issue #131 volatility benchmark runner combines `calibration`, `volatility`, and `changepoints` to compare arch GJR-GARCH/EGARCH challengers against the statsmodels Markov AR(2) baseline on a caller-supplied immutable CSV hash, writing only bounded canonical JSON summaries under `docs/evidence/`. The current #131 report is [docs/REGIME_VOLATILITY_CHALLENGERS.md](docs/REGIME_VOLATILITY_CHALLENGERS.md): artifact SHA `8cfcca8090169b3189b62e307e8be568cfd4fabeeff67e5b2d7055fc3d72602e`, decision `reject`, with promotion failed closed because the Markov AR(2) baseline did not converge. QuantLib process-global evaluation dates are handled only inside the internal `quantlib_state` adapter and do not cross public result/domain contracts.
